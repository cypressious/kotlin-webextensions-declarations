 ...
 * @param muted Whether the tab is prevented from playing sound (but hasn't necessarily recently produced sound). Equivalent to whether the muted audio indicator is showing.
 * @param reason The reason the tab was muted or unmuted. Not set if the tab's mute state has never been changed.
 * @param extensionId The ID of the extension that changed the muted state. Not set if an extension was not the reason the muted state last changed.
 */
class MutedInfo(
    var muted: Boolean,
 ...
 * @param screen If the tab is sharing the screen the value will be one of "Screen", "Window", or "Application", or undefined if not screen sharing.
...
/**
 * @param id The ID of the tab. Tab IDs are unique within a browser session. Under some circumstances a Tab may not be assigned an ID, for example when querying foreign tabs using the $(ref:sessions) API, in which case a session ID may be present. Tab ID can also be set to $(ref:tabs.TAB_ID_NONE) for apps and devtools windows.
 ...
 * @param openerTabId The ID of the tab that opened this tab, if any. This property is only present if the opener tab still exists.
 * @param highlighted Whether the tab is highlighted. Works as an alias of active
 * @param active Whether the tab is active in its window. (Does not necessarily mean the window is focused.)
 ...
 * @param audible Whether the tab has produced sound over the past couple of seconds (but it might not be heard if also muted). Equivalent to whether the speaker audio indicator is showing.
 * @param mutedInfo Current tab muted state and the reason for the last state change.
 * @param url The URL the tab is displaying. This property is only present if the extension's manifest includes the <code>"tabs"</code> permission.
 * @param title The title of the tab. This property is only present if the extension's manifest includes the <code>"tabs"</code> permission.
 * @param favIconUrl The URL of the tab's favicon. This property is only present if the extension's manifest includes the <code>"tabs"</code> permission. It may also be an empty string if the tab is loading.
 ...
 * @param sessionId The session ID used to uniquely identify a Tab obtained from the $(ref:sessions) API.
 ...
 * @param mode Defines how zoom changes are handled, i.e. which entity is responsible for the actual scaling of the page; defaults to <code>automatic</code>.
 * @param scope Defines whether zoom changes will persist for the page's origin, or only take effect in this tab; defaults to <code>per-origin</code> when in <code>automatic</code> mode, and <code>per-tab</code> otherwise.
 * @param defaultZoomFactor Used to return the default zoom level for the current tab in calls to tabs.getZoomSettings.
 */
class ZoomSettings(
 ...
 * @param shrinkToFit Whether the page content should shrink to fit the page width (overrides scaling). Default: true.
 ...
 * @param edgeLeft The spacing between the left header/footer and the left edge of the paper (inches). Default: 0.
 * @param edgeRight The spacing between the right header/footer and the right edge of the paper (inches). Default: 0.
 * @param edgeTop The spacing between the top of the headers and the top edge of the paper (inches). Default: 0
 * @param edgeBottom The spacing between the bottom of the footers and the bottom edge of the paper (inches). Default: 0.
 * @param marginLeft The margin between the page content and the left edge of the paper (inches). Default: 0.5.
 * @param marginRight The margin between the page content and the right edge of the paper (inches). Default: 0.5.
 * @param marginTop The margin between the page content and the top edge of the paper (inches). Default: 0.5.
 * @param marginBottom The margin between the page content and the bottom edge of the paper (inches). Default: 0.5.
 ...
 * @param urls A list of URLs or URL patterns. Events that cannot match any of the URLs will be filtered out.  Filtering with urls requires the <code>"tabs"</code> or  <code>"activeTab"</code> permission.
 * @param properties A list of property names. Events that do not match any of the names will be filtered out.
 */
class UpdateFilter(
...
/**
 * @param name Will be passed into onConnect for content scripts that are listening for the connection event.
 * @param frameId Open a port to a specific $(topic:frame_ids)[frame] identified by <code>frameId</code> instead of all frames in the tab.
 */
class ConnectInfo(
...
/**
 * @param frameId Send a message to a specific $(topic:frame_ids)[frame] identified by <code>frameId</code> instead of all frames in the tab.
 */
class Options(
    var frameId: Int? = null
)
...
/**
 * @param windowId The window to create the new tab in. Defaults to the $(topic:current-window)[current window].
 * @param index The position the tab should take in the window. The provided value will be clamped to between zero and the number of tabs in the window.
 * @param url The URL to navigate the tab to initially. Fully-qualified URLs must include a scheme (i.e. 'http://www.google.com', not 'www.google.com'). Relative URLs will be relative to the current page within the extension. Defaults to the New Tab Page.
 * @param active Whether the tab should become the active tab in the window. Does not affect whether the window is focused (see $(ref:windows.update)). Defaults to <var>true</var>.
 * @param pinned Whether the tab should be pinned. Defaults to <var>false</var>
 * @param openerTabId The ID of the tab that opened this tab. If specified, the opener tab must be in the same window as the newly created tab.
 ...
 * @param url Match tabs against one or more $(topic:match_patterns)[URL patterns]. Note that fragment identifiers are not matched.
 * @param windowId The ID of the parent window, or $(ref:windows.WINDOW_ID_CURRENT) for the $(topic:current-window)[current window].
 * @param windowType The type of window the tabs are in.
 * @param index The position of the tabs within their windows.
 * @param cookieStoreId The CookieStoreId used for the tab.
 * @param openerTabId The ID of the tab that opened this tab. If specified, the opener tab must be in the same window as this tab.
 ...
 * @param active Whether the tab should be active. Does not affect whether the window is focused (see $(ref:windows.update)).
 ...
 * @param discarded True while the tab is not loaded with content.
 * @param favIconUrl The tab's new favicon URL. This property is only present if the extension's manifest includes the <code>"tabs"</code> permission.
 ...
 * @param title The title of the tab if it has changed. This property is only present if the extension's manifest includes the <code>"tabs"</code> permission.
 * @param url The tab's URL if it has changed. This property is only present if the extension's manifest includes the <code>"tabs"</code> permission.
 */
class ChangeInfo(
    var attention: Boolean? = null,
    var audible: Boolean? = null,
...
external class TabsNamespace {
     ...
     */
    fun captureTab(tabId: Int? = definedExternally, options: ImageDetails? = definedExternally): Promise<Any>
