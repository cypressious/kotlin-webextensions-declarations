 ...
 * @param bookmarkId The id of the bookmark where the context menu was clicked, if it was on a bookmark.
 ...
 * @param targetElementId An identifier of the clicked element, if any. Use menus.getTargetElement in the page to find the corresponding element.
 */
class OnClickData(
 ...
 * @param id The unique ID to assign to this item. Mandatory for event pages. Cannot be the same as another ID for this extension.
 ...
 * @param checked The initial state of a checkbox or radio item: true for selected and false for unselected. Only one radio item can be selected at a time in a given group of radio items.
 * @param contexts List of contexts this menu item will appear in. Defaults to ['page'] if not specified.
 * @param visible Whether the item is visible in the menu.
 * @param onclick A function that will be called back when the menu item is clicked. Event pages cannot use this; instead, they should register a listener for $(ref:contextMenus.onClicked).
 * @param parentId The ID of a parent menu item; this makes the item a child of a previously added item.
 * @param documentUrlPatterns Lets you restrict the item to apply only to documents whose URL matches one of the given patterns. (This applies to frames as well.) For details on the format of a pattern, see $(topic:match_patterns)[Match Patterns].
 * @param targetUrlPatterns Similar to documentUrlPatterns, but lets you filter based on the src attribute of img/audio/video tags and the href of anchor tags.
 * @param enabled Whether this context menu item is enabled or disabled. Defaults to true.
 * @param command Specifies a command to issue for the context click.  Currently supports internal commands _execute_page_action, _execute_browser_action and _execute_sidebar_action.
 */
