 ...
 * @param danger Indication of whether this download is thought to be safe or known to be suspicious.
 ...
 * @param paused True if the download has stopped reading data from the host, but kept the connection open.
 * @param error Number indicating why a download was interrupted.
 * @param bytesReceived Number of bytes received so far from the host, without considering file compression.
 * @param totalBytes Number of bytes in the whole file, without considering file compression, or -1 if unknown.
 ...
 * @param query This array of search terms limits results to <a href='#type-DownloadItem'>DownloadItems</a> whose <code>filename</code> or <code>url</code> contain all of the search terms that do not begin with a dash '-' and none of the search terms that do begin with a dash.
 * @param startedBefore Limits results to downloads that started before the given ms since the epoch.
 ...
 * @param totalBytesGreater Limits results to downloads whose totalBytes is greater than the given integer.
 * @param totalBytesLess Limits results to downloads whose totalBytes is less than the given integer.
 * @param filenameRegex Limits results to <a href='#type-DownloadItem'>DownloadItems</a> whose <code>filename</code> matches the given regular expression.
 * @param urlRegex Limits results to <a href='#type-DownloadItem'>DownloadItems</a> whose <code>url</code> matches the given regular expression.
 * @param limit Setting this integer limits the number of results. Otherwise, all matching <a href='#type-DownloadItem'>DownloadItems</a> will be returned.
 * @param orderBy Setting elements of this array to <a href='#type-DownloadItem'>DownloadItem</a> properties in order to sort the search results. For example, setting <code>orderBy='startTime'</code> sorts the <a href='#type-DownloadItem'>DownloadItems</a> by their start time in ascending order. To specify descending order, prefix <code>orderBy</code> with a hyphen: '-startTime'.
 * @param url Absolute URL.
 * @param filename Absolute local path.
 * @param danger Indication of whether this download is thought to be safe or known to be suspicious.
 * @param mime The file's MIME type.
 ...
 * @param saveAs Use a file-chooser to allow the user to select a filename. If the option is not specified, the file chooser will be shown only if the Firefox "Always ask you where to save files" option is enabled (i.e. the pref <code>browser.download.useDownloadDir</code> is set to <code>false</code>).
 * @param method The HTTP method to use if the URL uses the HTTP[S] protocol.
 * @param headers Extra HTTP headers to send with the request if the URL uses the HTTP[s] protocol. Each header is represented as a dictionary containing the keys <code>name</code> and either <code>value</code> or <code>binaryValue</code>, restricted to those allowed by XMLHttpRequest.
...
/**
 * @param size The size of the icon.  The returned icon will be square with dimensions size * size pixels.  The default size for the icon is 32x32 pixels.
 */
class Options2(
    var size: Int? = null
)
 ...
 * @param url Describes a change in a <a href='#type-DownloadItem'>DownloadItem</a>'s <code>url</code>.
 * @param filename Describes a change in a <a href='#type-DownloadItem'>DownloadItem</a>'s <code>filename</code>.
 * @param danger Describes a change in a <a href='#type-DownloadItem'>DownloadItem</a>'s <code>danger</code>.
 * @param mime Describes a change in a <a href='#type-DownloadItem'>DownloadItem</a>'s <code>mime</code>.
 * @param startTime Describes a change in a <a href='#type-DownloadItem'>DownloadItem</a>'s <code>startTime</code>.
 * @param endTime Describes a change in a <a href='#type-DownloadItem'>DownloadItem</a>'s <code>endTime</code>.
 * @param state Describes a change in a <a href='#type-DownloadItem'>DownloadItem</a>'s <code>state</code>.
 * @param paused Describes a change in a <a href='#type-DownloadItem'>DownloadItem</a>'s <code>paused</code>.
 * @param error Describes a change in a <a href='#type-DownloadItem'>DownloadItem</a>'s <code>error</code>.
 * @param totalBytes Describes a change in a <a href='#type-DownloadItem'>DownloadItem</a>'s <code>totalBytes</code>.
 * @param fileSize Describes a change in a <a href='#type-DownloadItem'>DownloadItem</a>'s <code>fileSize</code>.
 */
class DownloadDelta(
    var id: Int,
