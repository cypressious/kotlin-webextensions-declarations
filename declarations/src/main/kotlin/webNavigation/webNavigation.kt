 ...
 * @param errorOccurred True if the last navigation in this frame was interrupted by an error, i.e. the onErrorOccurred event fired.
 * @param url The URL currently associated with this frame, if the frame identified by the frameId existed at one point in the given tab. The fact that an URL is associated with a given frameId does not imply that the corresponding frame still exists.
 * @param tabId The ID of the tab in which the frame is.
 * @param frameId The ID of the frame. 0 indicates that this is the main frame; a positive value indicates the ID of a subframe.
 ...
 * @param frameId 0 indicates the navigation happens in the tab content window; a positive value indicates navigation in a subframe. Frame IDs are unique for a given tab and process.
 * @param parentFrameId ID of frame that wraps the frame. Set to -1 of no parent frame exists.
 * @param timeStamp The time when the browser was about to start the navigation, in milliseconds since the epoch.
 */
class Details5(
    var tabId: Int,
    var url: String,
 ...
 * @param frameId 0 indicates the navigation happens in the tab content window; a positive value indicates navigation in a subframe. Frame IDs are unique within a tab.
 ...
 * @param tabId The ID of the tab in which the navigation occurs.
 * @param frameId 0 indicates the navigation happens in the tab content window; a positive value indicates navigation in a subframe. Frame IDs are unique within a tab.
 * @param timeStamp The time when the page's DOM was fully constructed, in milliseconds since the epoch.
 */
class Details7(
    var tabId: Int,
    var url: String,
 ...
 * @param sourceFrameId The ID of the frame with sourceTabId in which the navigation is triggered. 0 indicates the main frame.
 * @param url The URL to be opened in the new window.
 * @param tabId The ID of the tab in which the url is opened
 * @param timeStamp The time when the browser was about to create a new view, in milliseconds since the epoch.
 */
class Details10(
    var sourceTabId: Int,
